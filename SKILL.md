...
description: Identify, review, and fix security vulnerabilities in vibecoded full-stack apps built with Next.js, Vercel, and Supabase. Use when auditing or patching projects that use the Next.js App Router, Server Components, Server Actions, Route Handlers, Vercel deployments/environment variables/protection settings, Supabase Auth, Supabase SSR clients, Row Level Security policies, Storage, Edge Functions, or Postgres migrations.
---
...
- Supabase usage: `createClient`, `createServerClient`, service-role clients, migrations, seed SQL, storage policies, auth callbacks.
- Vercel usage: `vercel.json`, `next.config.* headers`, environment variable names, source maps, deployment protection assumptions.

Use `rg` patterns first:

```bash
rg -n "NEXT_PUBLIC_|SUPABASE|service_role|service-role|createClient|createServerClient|use server|auth\.uid|auth\.jwt|security definer|enable row level security|create policy|grant .* to anon|grant .* to authenticated|vercel|headers\(" .
```
